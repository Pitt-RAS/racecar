...
def best_fit(x_points,y_points):
	...
	m = (((mean(x_points)*mean(y_points)) - mean(x_points*y_points))/((mean(x_points)*mean(x_points))-mean(x_points*x_points)))
...
while True:
    ...
    linesP = cv2.HoughLinesP(abs_dst, rho = 6, theta = np.pi / 60, threshold = 50, lines=np.array([]), minLineLength= 30, maxLineGap = 10)
